...
export const DIMENSOES_PADRAO: Dimensao[] = [
...
export default function Scorecard({ scorecard = {}, onSalvar, colunas = 1 }: Props) {
  ...
  const gridClass = colunas === 3 ? 'grid grid-cols-1 md:grid-cols-3 gap-4' :
                    colunas === 2 ? 'grid grid-cols-1 md:grid-cols-2 gap-4' :
                    'space-y-4'
    ...
    <div className="space-y-4">
      ...
      <div className={`p-4 rounded-xl border-2 ${veredito.cor}`}>
        ...
        <div className="h-2 bg-white/50 rounded-full overflow-hidden">
          <div className="h-full bg-current rounded-full transition-all duration-300" style={{ width: `${pct}%` }} />
        </div>
